...
GitHub Desktop does not configure credentials for our LFS server at `{{lfsServer}}`. After signing in, complete the setup using either [`gh`](gh-cli.md) (recommended) or [`git-credential-manager`](git-credential-manager.md).
:::
